...
exports.handler = async function(event) {
  console.log(event)
  const { guild_id, schedulesUrl, teamsUrl, messageToken } = JSON.parse(
    event.body
  )
  const teamsFetch = fetch(teamsUrl, {
    headers: {
      "User-Agent":
        "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.79 Safari/537.36",
    },
  })
  ...
  const [teamsData, schedulesData] = await Promise.all([
    teamsFetch.then(res => res.json()),
    schedulesFetch.then(res => res.json()),
  ])
  ...
  Object.keys(teamsData).map(teamId => {
    teams[teamId] = {
      teamName: teamsData[teamId].displayName,
      abbr: teamsData[teamId].abbrName,
    }
  })
  ...
  try {
    ...
    const res = await DiscordRequest(
      `webhooks/${process.env.APP_ID}/${messageToken}/messages/@original`,
      {
        method: "PATCH",
        body: {
